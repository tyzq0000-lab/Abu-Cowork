...
export async function completeEmployeeDeployment(
  ...
  useEmployeeDeploymentStore.getState().saveDeployment({
    ...
    platformDisplayName: input.platformBinding?.displayName ?? deployment?.platformDisplayName,
    platformProfession: input.platformBinding?.profession ?? deployment?.platformProfession,
    platformAvatar: input.platformBinding?.avatar ?? deployment?.platformAvatar,
