...
### B4. 铁律 (Iron Rules) — 最高优先级，不可违反

以下三条优先级高于本文件其余所有约定。与任何"优化"冲突时，铁律胜出。

1. **只增不减**：扶摇桌面端功能在 fork 的 abu-cowork 基础上**只增不减**；任何会减少 / 损坏 / 冲突 abu-cowork 既有功能的调整，**必须先向用户确认同意**。
2. **三端独立性**：扶摇桌面端 / uprow 平台 / 数字员工包 各自保持通用性与独立性，**不为某一项功能削减通用性**——目标是适配更多职业岗位的数字员工包（任何"映射层 / 适配层"做成通用机制，不为单一员工包定制）。
3. **对齐上游**：及时对齐上游 abu-cowork 更新（监测机制已建，见 `UPSTREAM-SYNC.md`）。
